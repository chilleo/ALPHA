...
def equality_sets(species_trees, network, taxa, gene_trees):
    """
...
def plot_formatting(files_to_l, verbose=False):
    """
    ...
    num = 0
    file_name = "GeneralizedDResultsByFile{0}.txt".format(num)
    while os.path.exists(file_name):
        num += 1
        file_name ="GeneralizedDResultsByFile{0}.txt".format(num)

    with open(file_name, "w") as text_file:
        for idx in files_to_l:
            info = files_to_l[idx]
            l_stat = info[0]
            significant = info[1]
...
def mosquito_analysis(directory, species_tree, reticulations):
    """
    Calculates the L statistic for the given alignment
    Input:
        directory --- the directory of phylip files to use
        species_tree --- the inputted species tree over the given taxa
        reticulations --- a tuple containing two dictionaries mapping the start leaves to end leaves
        verbose --- a boolean for determining if extra information will be printed
    Output:
    ...
    #Map each partial chromosome file to its generalized D value
    files_to_l = {}

    # The calculation of the statistic is constant for each file
    st = re.sub("\:\d+\.\d+", "", species_tree)
    trees, taxa = branch_adjust(st)
    outgroup = taxa[-1]
    gene_trees = generate_unique_trees(taxa, outgroup)
    network = generate_network_tree((0.1, 0.9), list(trees)[0], reticulations)
    trees_to_equality, trees_to_equality_N, patterns_pgS, patterns_pgN = equality_sets(trees, network, taxa, gene_trees)
    trees_of_interest = set_of_interest(trees_to_equality, trees_to_equality_N)
    increase, decrease = determine_patterns(trees_of_interest, trees_to_equality, patterns_pgN)

    file_order = 0

    # Iterate over each alignment file in the folder in a naturally sorted order
    for alignment_file in natsorted(os.listdir(directory)):

        # Only use alignment files in phylip format
        if os.path.splitext(alignment_file)[1] == ".phylip":

            # Rename the alignment file to its global location
            alignment_file = os.path.join(directory, alignment_file)

            l_stat, significant = calculate_L(alignment_file, taxa, (increase, decrease))
            print l_stat
            print significant
            files_to_l[file_order] = (l_stat, significant)
            file_order += 1
...
if __name__ == '__main__':
    ...
    # Inputs for paper
    directory = "C:\\Users\\travi\\Desktop\\3L\\Mosquito"
    species_tree = '((C,G),(((A,Q),L),R));'
    r = [('L', 'R')]
    plot_formatting(mosquito_analysis(directory, species_tree, r))
